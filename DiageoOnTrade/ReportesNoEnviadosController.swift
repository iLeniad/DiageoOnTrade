//
//  ReportesNoEnviadosController.swift
//  OnTrade2
//
...
class ReportesNoEnviadosController: UIViewController,UIScrollViewDelegate,CLLocationManagerDelegate,GMSMapViewDelegate {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        textoSubTitulo.setTitle("Reportes No Enviados", for: UIControlState())
    ...
    func mostrar_reportes_enviados(){
        ...
        let sqlReportesEnviados = "select t1.*,t2.name from report t1, pdv_pdv t2 where t1.enviado = 0 and idTipo <> '-1' and t1.place = t2.id order by id desc"
        ...
        for renglonReportesEnviados in resultadoReportesEnviados {
            ...
            botonReportesEnviados.frame = CGRect(x: offsetx, y: offsetScroll, width: (laVista.frame.width/5)*3, height: alto_menu)
            
            botonReportesEnviados.titleLabel!.adjustFontSizeToFitRect(rect: botonReportesEnviados.frame, maximo: tamano_letra_menu)
            ...
            let botonBorrar:UIButton = UIButton()
            
            botonBorrar.tag = renglonReportesEnviados["id"] as! Int
            
            botonBorrar.titleLabel!.font = UIFont(name: fontFamilia, size: CGFloat(3))
            
            botonBorrar.titleLabel!.font = botonBorrar.titleLabel!.font.withSize(CGFloat(tamano_letra_menu))
            
       
            botonBorrar.setAttributedTitle(nil, for: UIControlState())
            botonBorrar.setTitle("borrar", for: UIControlState())
            botonBorrar.tag = renglonReportesEnviados["id"] as! Int
            
            botonBorrar.isSelected = false
            botonBorrar.setTitleColor(UIColor(rgba: "#\(color_letra)"), for: UIControlState())
            
            
            
            botonBorrar.titleLabel!.textColor = UIColor(rgba: "#\(color_letra)")
            botonBorrar.titleLabel!.numberOfLines = 0
            botonBorrar.titleLabel!.textAlignment = .left
            
            
            botonBorrar.sizeToFit()
            
            botonBorrar.frame = CGRect(x: (laVista.frame.width/4)*3, y: offsetScroll, width: laVista.frame.width/4, height: alto_menu)
            
            
            
            botonBorrar.contentHorizontalAlignment = .left
            botonBorrar.contentVerticalAlignment = .center
            
            
            botonBorrar.addTarget(self, action:#selector(ReportesNoEnviadosController.borrar_reporteNoEnviado(sender:)), for:.touchDown)
            
            laVista.addSubview(botonBorrar)
    ...
    //borrar reporte no enviado
    
    @objc func borrar_reporteNoEnviado(sender:UIButton){
        
        
        
        
        // Create the alert controller
        let alertController = UIAlertController(title: "Atención", message: "¿Esta seguro que desea borrar el reporte?", preferredStyle: .alert)
        ...
        let okAction = UIAlertAction(title: "Aceptar", style: UIAlertActionStyle.default) {
            ...
            sql = "delete from report where id = '\(sender.tag)'"
            
            _ = self.db.execute_query(sql)
            
            sql = "delete from report_distribution where idReporteLocal = '\(sender.tag)'"
            
            _ = self.db.execute_query(sql)
            
            sql = "delete from report_photo_distribution where idReporteLocal = '\(sender.tag)'"
            
            _ = self.db.execute_query(sql)
            
            sql = "delete from report_photo_promotions where idReporteLocal = '\(sender.tag)'"
            
            _ = self.db.execute_query(sql)
            
            sql = "delete from report_photo_visibility where idReporteLocal = '\(sender.tag)'"
            
            _ = self.db.execute_query(sql)
            
            sql = "delete from report_promotions where idReporteLocal = '\(sender.tag)'"
            
            _ = self.db.execute_query(sql)
            
            sql = "delete from report_promtions_brands where idReporteLocal = '\(sender.tag)'"
            
            _ = self.db.execute_query(sql)
            
            sql = "delete from report_visibility where idReporteLocal = '\(sender.tag)'"
            
            _ = self.db.execute_query(sql)
            
            sql = "delete from report_visibility_brands where idReporteLocal = '\(sender.tag)'"
            
            _ = self.db.execute_query(sql)
            
            self.mostrar_reportes_enviados()
